...
class Neural:
    def __init__(self, learn_rate=0.01):
        self.X = np.zeros((112, 7))
        self.Y = np.zeros((112, 4))
        self.learn_rate = learn_rate
        with open("BPmeituanData.csv", encoding='utf8') as f:
            reader = csv.reader(f)
            next(reader)
            i = 0
            for row in reader:
    ...
    def backward_propagation(self, parameters, cache):
        ...
        dz1 = np.multiply(dz2.dot(w2.T), sigmoid_d(Z1))
